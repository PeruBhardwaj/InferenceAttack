...
#     - Select entity substitution that is ranked just worse than target triple
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/inv_add_2/{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, args.target_split, args.budget, args.rand_run)
    ...
    for test_idx, triple in enumerate(test_data):
        ...
        pred_s = model.forward(o,r_rev, mode='lhs', sigmoid=False) #this gives scores not probabilities
        ...
        for bud in range(args.budget):
            o_dash = argsort_o[rank_o+1+bud]
            s_dash = argsort_s[rank_s+1+bud]
    ...
    save_path = 'data/inv_add_2_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, args.target_split, args.budget, args.rand_run)
    ...
    with open(os.path.join(save_path, 'stats.txt'), 'w') as f:
        ...
        f.write('This attack version is generated using just worse rank than target triple \n')
        f.write('---------------------------------------------------------------------- \n')

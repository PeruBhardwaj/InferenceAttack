...
# - In this notebook - 
#     - inverse relations are selected using r1.r2 =1 
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/inv_add_1/{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, args.target_split, args.budget, args.rand_run)
    ...
    # Grounding for inversion is $\mathcal{G}_d: (o', \mathtt{r}_i, s) \Rightarrow (s, \mathtt{r}, o')$ and $\mathcal{G}_d: (o, \mathtt{r}_i, s') \Rightarrow (s', \mathtt{r}, o)$.
    # 
    # The soft truth score is $\phi(\mathcal{G}_i) = \phi(o',r_i,s) \cdot \phi(s,r,o') - \phi(o',r_i,s) + 1$
    # 
    # and $\phi(\mathcal{G}_i) = \phi(o,r_i,s') \cdot \phi(s',r,o) - \phi(o,r_i,s') + 1$
    ...
    for test_idx, triple in enumerate(test_data):
        ...
        # compute candidate scores for (o',ri,s)
        candidate_truth = model.forward(s,r_inv,mode='lhs',sigmoid=True).squeeze()
        ...
        o_dash_all = torch.topk(ground_truth, k=args.budget, largest =False).indices
        ...
        # compute candidate scores for (o,ri,s')
        candidate_truth = model.forward(o,r_inv,mode='rhs',sigmoid=True).squeeze()
        ground_truth = candidate_truth*target_truth
        ground_truth += 1

        ground_truth[filter_s] = 1e6
        ground_truth[s] = 1e6 # we don't want to select (s,r,o) as edit
        s_dash_all = torch.topk(ground_truth, k=args.budget, largest =False).indices
        ...
        for bud in range(args.budget):
            s_dash, o_dash = s_dash_all[bud].item(), o_dash_all[bud].item()
    ...
    save_path = 'data/inv_add_1_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, args.target_split, args.budget, args.rand_run)

###  Add triples based on IF triple, chosen as gradient similarity - cos, dot, l2
...
#         r_rev = r
    ...
    label =  torch.from_numpy(np.ones(batch.shape[0])).to(device)
    pred = model.score_triples(s,r,o)
    train_loss = model.loss(pred, label) ## model loss is BCE with logits
    
#     pred_sr = model.forward(s,r, mode='rhs')
#     loss_sr = model.loss(pred_sr, o) # loss is cross entropy loss

#     pred_or = model.forward(o, rrev, mode='lhs')
...
def get_if_triple(test_trip, nghbr_trip, model, attack_batch_size, simmetric, param_influence):
    ...
    for train_trip in nghbr_trip:
        ...
        if simmetric == 'l2':
            dist = -torch.norm((test_grads-train_grads), p=2, dim=-1)
        elif simmetric == 'dot':
            dist = torch.matmul(test_grads, train_grads) #default dim=1
        else: ##cos
            dist = F.cosine_similarity(test_grads, train_grads, dim=0) #default dim=1
        nghbr_dist.append(dist.item())
...
def get_additions(train_data, test_data, neighbours, model, attack_batch_size, simmetric):
    ...
    for test_idx, test_trip in enumerate(test_data):
        ...
        del_idx_1, del_idx_2 = get_if_triple(test_trip, nghbr_trip, model, attack_batch_size, simmetric, param_influence)
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/grad_add_{5}/{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
    ...
    save_path = 'data/grad_add_{5}_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, args.target_split, args.budget, args.rand_run,
                                                               args.sim_metric
                                                              )
    try :
    ...
    with open(os.path.join(save_path, 'stats.txt'), 'w') as f:
        ...
        f.write('Instance Attribution Attacks (Gradients) - This attack version is generated uses gradient similarity metric: {0} \n'.format(args.sim_metric))

...
#     - select decoy triple that is ranked just worse than target triple
...
log_path = 'logs/attack_logs/com_add_2/{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, args.target_split, args.budget, args.rand_run)    
...
# Choosing the decoy triple that is ranked just worse than the target triple
...
for test_idx, triple in enumerate(test_data):
    ...
    pred_s = model.forward(o,r, mode='lhs', sigmoid=False) #this gives scores not probabilities
    pred_o = model.forward(s,r, mode='rhs', sigmoid=False) # this gives scores not probabilities
    
    pred_s, pred_o = pred_s.squeeze(), pred_o.squeeze()
    ...
    for bud in range(args.budget):
        if rank_o + 1 >= argsort_o.shape[0]:  # this happens for nations TransE
            ...
            rank_o = r_int - 1
        o_dash = argsort_o[rank_o+1+bud][None]
        ...
        s_dash = argsort_s[rank_o+1+bud][None]
        # filters are (s',r1,?) and (?,r2, o) 
...
save_path = 'data/com_add_2_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, args.target_split, args.budget, args.rand_run)
...
with open(os.path.join(save_path, 'stats.txt'), 'w') as f:
    ...
    f.write('This attack version is generated for decoy chosen via just worse ranks \n')
    f.write('---------------------------------------------------------------------- \n')
